...
|-|-|-|
|Orciyami|JiJeq-Orgex|2026|
|LimaDice|JiJeq-Orgex/Novek|2026|
|GpEi|JiJeq-Orgex|N/A|
|Etude de DQre WoodHouse|JiJeq-Orgex|N/A|
|Antway|JiJeq-Orgex|N/A|
...
# File Format:
    ...
    <<START OF Rolleq FORMAT>>
    "ROLLEQ" (Header 6 bytes)
    checksum (8 bytes)
    >Version (1 byte)
    >chip (1 byte)
    >bank (1 byte)
    ...
    <<END OF Rolleq FORMAT>>

    then compress it in zlib.

    use your custom key and then compress it in zlib. (for final song's release only, won't work on actual music software)
...
### Custom Assembly Language:
    ...
    - UQ [unique type number imm/reg ] [unique obj number 16-bit imm/reg/mem1-4] //Unique variables 0x200-0x23f (16-bit for each / 1st ram only):
    e.g. UQ 0-31 0xffff
      - 0 //Active (0x200-0x201) [//Turn on/off unique object via active (unused)]
      - 1 //Looping (0x202-0x203)
      - 2 //Delay (0x204-0x205)
      - 3 //Shutdown (0x206-0x207)
      - 4 //Random (0x208-0x209)
      - 5 //SOFF (0x20A-0x20B)
      - 6 //EOFF (0x20C-0x20D)
      - 7 //Channel  (0x20E-0x20F)
      - 8 //Once (0x210-0x211)
      - 9 // (0x212-0x213)
      - 10 // (0x214-0x215)
      - 11 // (0x216-0x217)
      ...
      - 29 // (0x23A-0x23B)
      - 30 // (0x23C-0x23D)
      - 31 //Main Value (0x23E-0x23F)

    - UNIQ_SEND 0-31 0x0000 //send seleted UQ memory datas to unique hex object id for changes
      - E.G. UNIQ_SEND [unique type number imm/reg ] [unique obj number 16-bit imm/reg/mem1-4]
